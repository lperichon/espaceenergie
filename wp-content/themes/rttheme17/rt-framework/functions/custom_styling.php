...
function rt_custom_styling(){
	...
	if($rttheme_link_color){
		echo '.content a, .sidebar a, #footer .box.footer.widget a,.tweet_time a, .box .tweet_text a, .banner .featured_text a, a.read_more{color:'.$rttheme_link_color.';}'; 
	}
	if($rttheme_link_color_hover){
		echo '.content a:hover, .sidebar a:hover, #footer .box.footer.widget a:hover,.tweet_time a:hover, .box .tweet_text a:hover, .banner .featured_text a:hover, a.read_more:hover {color:'.$rttheme_link_color_hover.';text-shadow:none;}'; 
	}
...
global $google_fonts; 
	...
	$the_query = new WP_Query('meta_value=custom_styled&post_type=home_page');
	$style = "";
