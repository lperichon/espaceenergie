...
class RTThemeAdmin extends RTTheme{
	...
	function rt_admin_ajax() {
		...
		}elseif(@!$_POST['saveoptions'] && $_POST['rt_theme_gallery']){ // returns new gallery form
		...
		foreach($options as $k => $v){
			...
			switch ($v['type']){
				...
				if($field_value){
					...
					echo '<div class="form_element check"><img class="loadit" src="'.$field_value.'"  id="'.$v['id'].'-show"></div>';
					echo '<img src="'.THEMEADMINURI.'/images/delete.png" class="delete '.$v['id'].'" id="delete_'.$v['id'].'">';			
					echo '</td></tr>';				
				}else{
					echo '<tr style="display:none;"><td class="col2" colspan="2">';
					echo '<div class="form_element check"><img class="loadit" src="'.THEMEADMINURI.'/images/blank.png"  id="'.$v['id'].'-show"></div>';
					echo '<img src="'.THEMEADMINURI.'/images/delete.png" class="delete '.$v['id'].'" id="delete_'.$v['id'].'">';			
					echo '</td></tr>';				
				} 
				...
				if(!empty($fontDemo)){
				//font-family name
				$font_family_name = @$this->google_fonts[$field_value][0];
				echo '    <tr>';
				echo '	<td class="col1" colspan="2">';
				...
				echo '	<select name="'.$v['id'].'" id="'.$v['id'].'" class="'.$class.' '.$fontSystem.' '. $extraClass .' ">';
					...
					foreach($v['options'] as $option_value => $option_name){					
						...
						if(is_array($option_name)){
							$option_name = $option_name[1];
							//font-family name
							$font_family_name = $this->google_fonts[$option_value][0]; 
						}
