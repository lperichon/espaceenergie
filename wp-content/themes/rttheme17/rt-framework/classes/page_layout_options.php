	...
	function rt_save_page_templates(){
		...
		foreach($savedTemplates as $key=>$value){
			...
			if(!$jump){
				...
				$templates->templates[$template]->contents[$box]->ajax_scroller      =  @$value["ajax_scroller"]; 
				...
				$templates->templates[$template]->contents[$box]->layout             =  $value["layout"]; 
					...
					//wpml register string
					wpml_register_string( THEMESLUG , 'Heading'.$templates->templates[$template]->templateID, $value["heading"] );
				}				
				//blog posts
	...
	function rt_generate_slider_box($theGroupID,$theTemplateID,$options,$randomClass){
			...
			$options = array (
					...
					array(
							...
							"desc" 		=> __("If you turn on the crop feature, the image will be cropped as the height value above.",'rt_theme_admin'),
							"hr"			=> true, 
							"class"		=> $randomClass,
