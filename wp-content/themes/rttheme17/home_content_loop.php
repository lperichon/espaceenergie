...
global $home_page,$which_theme,$row,$layout_values,$layout,$firstBox,$lastBox,$reset,$this_column_width_pixel,$item_width,$box_border,$layout_values,$content_width,$container_border;
		...
		$featured_image_position	=	@get_post_meta($post->ID, THEMESLUG.'featured_image_position', true);
		$heading_and_text_position	=	@get_post_meta($post->ID, THEMESLUG.'heading_and_text_position', true);
		$hide_the_heading		=	@get_post_meta($post->ID, THEMESLUG.'hide_the_heading', true);
		...
		*/
		...
		<!-- box -->
		<div class="template_builder box home-content-box box-shadow <?php echo $layout;?> <?php echo $firstBox .' '. $lastBox  .' '. $addClass2;?>" id="post-<?php echo $post->ID;?>">
